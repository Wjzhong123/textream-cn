...
enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var label: String {
        ...
        case .guidance:   return LocalizedStrings.guidance
        case .teleprompter: return LocalizedStrings.teleprompter
...
struct SettingsView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Button(LocalizedStrings.resetAll) {
        ...
        .alert(LocalizedStrings.resetAllSettings, isPresented: $showResetConfirmation) {
            ...
            Text(LocalizedStrings.resetWarning)
        }
        .onAppear {
    ...
    private var appearanceTab: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                // Highlight Color
                Text(LocalizedStrings.highlightColor)
                    .font(.system(size: 13, weight: .medium))
                ...
                // Dimensions
                Text(LocalizedStrings.dimensions)
                    .font(.system(size: 13, weight: .medium))

                VStack(spacing: 10) {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text(LocalizedStrings.width)
                                .font(.system(size: 12))
                                .foregroundStyle(.secondary)
                            Spacer()
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if settings.listeningMode == .wordTracking {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(LocalizedStrings.speechLanguage)
                        .font(.system(size: 13, weight: .medium))
            ...
            if settings.listeningMode != .classic {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(LocalizedStrings.microphone)
                        .font(.system(size: 13, weight: .medium))
                    Picker("", selection: $settings.selectedMicUID) {
                        Text(LocalizedStrings.systemDefault).tag("")
            ...
            if settings.listeningMode != .wordTracking {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack {
                        Text(LocalizedStrings.slower)
                            .font(.system(size: 10))
                            .foregroundStyle(.tertiary)
                        Spacer()
                        Text(LocalizedStrings.faster)
                            .font(.system(size: 10))
                            .foregroundStyle(.tertiary)
                    }
                }
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if settings.overlayMode == .floating {
                    ...
                    Toggle(isOn: $settings.followCursorWhenUndocked) {
                        Text(LocalizedStrings.followCursor)
                            .font(.system(size: 13, weight: .medium))
                    }
                    .toggleStyle(.switch)
                    .controlSize(.small)

                    Text(LocalizedStrings.followCursorDesc)
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                    ...
                    Toggle(isOn: $settings.floatingGlassEffect) {
                        Text(LocalizedStrings.glassEffect)
                            .font(.system(size: 13, weight: .medium))
                    }
                    .toggleStyle(.switch)
                    .controlSize(.small)
                ...
                if settings.overlayMode == .fullscreen {
                    ...
                    HStack(spacing: 6) {
                        Image(systemName: "escape")
                            .font(.system(size: 11, weight: .semibold))
                            .foregroundStyle(.secondary)
                        Text(LocalizedStrings.pressEscToStop)
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                    }
                    .padding(10)
                ...
                Toggle(isOn: $settings.showElapsedTime) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LocalizedStrings.elapsedTime)
                            .font(.system(size: 13, weight: .medium))
                        Text(LocalizedStrings.elapsedTimeDesc)
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                Toggle(isOn: $settings.hideFromScreenShare) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LocalizedStrings.hideFromScreenSharing)
                            .font(.system(size: 13, weight: .medium))
                        Text(LocalizedStrings.hideFromScreenSharingDesc)
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                // Pagination
                Text(LocalizedStrings.pagination)
                    .font(.system(size: 13, weight: .semibold))

                Toggle(isOn: $settings.autoNextPage) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LocalizedStrings.autoNextPage)
                            .font(.system(size: 13, weight: .medium))
                        Text(LocalizedStrings.autoNextPageDesc)
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                if settings.autoNextPage {
                    HStack {
                        Text(LocalizedStrings.countdown)
                            .font(.system(size: 13))
                        Spacer()
                        Picker("", selection: $settings.autoNextPageDelay) {
                            Text(LocalizedStrings.seconds3).tag(3)
                            Text(LocalizedStrings.seconds5).tag(5)
                        }
                        .pickerStyle(.segmented)
                        .frame(width: 160)
                    }
    ...
    private var externalTab: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(LocalizedStrings.externalDisplayDesc)
                .font(.system(size: 11))
                .foregroundStyle(.secondary)
            ...
            if settings.externalDisplayMode == .mirror {
                ...
                Text(LocalizedStrings.mirrorAxis)
                    .font(.system(size: 13, weight: .medium))
            ...
            if settings.externalDisplayMode != .off {
                ...
                Text(LocalizedStrings.targetDisplay)
                    .font(.system(size: 13, weight: .medium))
                ...
                displayPicker(
                    ...
                    emptyMessage: LocalizedStrings.noExternalDisplays
                )
            }
            Spacer()
        }
    ...
    private var browserTab: some View {
        ScrollView(.vertical, showsIndicators: false) {
        VStack(alignment: .leading, spacing: 14) {
            Text(LocalizedStrings.remoteDesc)
                .font(.system(size: 11))
                .foregroundStyle(.secondary)

            Toggle(isOn: $settings.browserServerEnabled) {
                Text(LocalizedStrings.enableRemoteConnection)
                    .font(.system(size: 13, weight: .medium))
            }
            .toggleStyle(.switch)
            .controlSize(.small)
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                Text(LocalizedStrings.restartRequired)
                                    .font(.system(size: 11))
                                    .foregroundStyle(.tertiary)
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "info.circle")
                                .font(.system(size: 11))
                                .foregroundStyle(.secondary)
                            Text(String(format: LocalizedStrings.portUsage, settings.browserServerPort, settings.browserServerPort + 1))
                                .font(.system(size: 11))
                                .foregroundStyle(.secondary)
                        }
                    }
    ...
    private var directorTab: some View {
        ScrollView(.vertical, showsIndicators: false) {
        VStack(alignment: .leading, spacing: 14) {
            Text(LocalizedStrings.directorDesc)
                .font(.system(size: 11))
                .foregroundStyle(.secondary)

            Toggle(isOn: $settings.directorModeEnabled) {
                Text(LocalizedStrings.enableDirectorMode)
                    .font(.system(size: 13, weight: .medium))
            }
            .toggleStyle(.switch)
            .controlSize(.small)
            ...
            if settings.directorModeEnabled {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "info.circle")
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                    Text(LocalizedStrings.directorWordTrackingForced)
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "info.circle")
                                .font(.system(size: 11))
                                .foregroundStyle(.secondary)
                            Text(String(format: LocalizedStrings.portUsage, settings.directorServerPort, settings.directorServerPort + 1))
                                .font(.system(size: 11))
                                .foregroundStyle(.secondary)
                        }
                    }

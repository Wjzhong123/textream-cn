...
class SpeechRecognizer {
    ...
    private func requestMicrophoneAccessAndBegin(for generation: Int) {
        ...
        case .denied, .restricted:
            failListening(LocalizedStrings.micAccessDenied)
...
self.failListening(LocalizedStrings.micAccessDenied)
                    }
                }
            }
        case .authorized:
    ...
    private func requestSpeechAuthAndBegin(for generation: Int) {
        SFSpeechRecognizer.requestAuthorization { [weak self] status in
            DispatchQueue.main.async {
                ...
                default:
self.failListening(LocalizedStrings.speechNotAuthorized)
...
if requiresSpeechRecognition {
            ...
            guard let speechRecognizer else {
                failListening(LocalizedStrings.speechNotAvailable)
                return
            }
...
failListening(LocalizedStrings.audioInputUnavailable)
            }
            return
        }
...
failListening(String(format: LocalizedStrings.audioEngineFailed, error.localizedDescription))
            }
        }
    }
